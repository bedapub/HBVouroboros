...
infref_strain_dup_FASTA = "results/inpt/inpt_strain_dup.fasta"
inpt_strain_FASTA = "results/inpt/inpt_strain.fasta"
...
rule bowtie2_index_ref_dup_perSmap:
    	...
    	output: "results/perSamp/{sample}/bowtie2_index"
    	threads: 1
...
rule infref_bowtie2_map_perSamp:
    	input:
        	genome = "results/perSamp/{sample}/bowtie2_index",
        	f1 = lambda wildcards: fq1dict[wildcards.sample],
        	f2 = lambda wildcards: fq2dict[wildcards.sample]
   	output:
...
rule genome_count_perSamp:
    	input:
        	"results/perSamp/{sample}/bam/{sample}.sorted.bam.stat"
    	output:
        	"results/perSamp/{sample}/coverage/genome_count.tsv"
    	shell:
         	"echo -e 'ID\tcoverage' > {output}; "
         	"grep -H '^SN' {input} | \
          	grep '1st fragments' | \
...
rule dupconc_depth_perSamp:
    	input:
        	...
        	bai = "results/perSamp/{sample}/bam/{sample}.sorted.bam.bai"
    	output:
       		temp("results/perSamp/{sample}/coverage/genome_dupconc_depth.tsv")
    	shell:
        	"samtools depth -a -H -d 0 {input.bam} -o {output}"
...
rule depth_perSamp:
    	input:
       		"results/perSamp/{sample}/coverage/genome_dupconc_depth.tsv"
    	output:
      		"results/perSamp/{sample}/coverage/genome_depth.tsv"
    	run:
       		dedup_file(input[0], output[0])
       		dedup_file(input[0], output[0])
...
rule dup_gff_perSamp:
    	...
    	output:
       		gff = "results/perSamp/{sample}/strain_dup.gff"
    	run:
       		dup_gff(input.fasta, input.gff, output.gff)
...
rule individual_coverage_perSamp:
    	input:
     		...
     		gff = "results/perSamp/{sample}/strain_dup.gff"
    	output:
      		"results/perSamp/{sample}/coverage/genome_feature_coverage.tsv"
   	shell:
        	"coverageBed -counts -a {input.gff} -b {input.bam} > {output}"
